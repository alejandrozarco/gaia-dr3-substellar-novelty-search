...
const verdict = await agent(
  ...
  `ALSO emit, for the object journal (docs/object_journals/${SID}.md): journal_entry = a dated markdown entry body (what was checked / found incl. nulls / provenance); ledger_rows = one per catalog or method checked, each {catalog, query, result, provenance} — record NULL/NOT-IN results too (esp. Shahaf+2023 / SIMBAD / literature membership: the exact thing that must never be reconstructed from memory).`,
  { label: 'synthesis', phase: 'Verdict', schema: { type: 'object', properties: { verdict: { type: 'string' }, M2: { type: 'string' }, P_compact: { type: 'string' }, is_novel: { type: 'boolean' }, decisive_facts: { type: 'array', items: { type: 'string' } }, caveats: { type: 'string' }, followup: { type: 'string' }, journal_entry: { type: 'string' }, ledger_rows: { type: 'array', items: { type: 'object', properties: { catalog: { type: 'string' }, query: { type: 'string' }, result: { type: 'string' }, provenance: { type: 'string' } }, required: ['catalog', 'result'] } } }, required: ['verdict', 'M2', 'is_novel'] } }
)

log(`${SID}: ${verdict.verdict} (novel=${verdict.is_novel}) — main thread: append verdict.journal_entry + ledger_rows via scripts/journal/journal.py`)
