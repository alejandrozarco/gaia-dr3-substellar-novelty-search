export const meta = {
  ...
  description: 'Pre-release integrity gate: whole-catalog overclaim sweep + test suite + git/sync check + README/CITATION/CANDIDATES consistency. Returns GO / NO-GO + a fix checklist. (Would have caught the README/CITATION "confirmed" misses.)',
...
phase('Checks')
const [overclaims, tests, gitst, consistency] = await parallel([
...
phase('Gate')
const gate = await agent(`Release gate decision. Overclaims: ${JSON.stringify(overclaims)}. Tests: ${JSON.stringify(tests)}. Git: ${JSON.stringify(gitst)}. Consistency: ${JSON.stringify(consistency)}. Decide GO or NO-GO. NO-GO if any stale overclaim, any failing test, an unclean/unpushed tree, or a headline inconsistency. Give the exact blocking items and a fix checklist (edits to apply in the main thread). Reminder: never delete a DOI'd release — supersede it with a banner + a new version.`, { label: 'gate', phase: 'Gate', schema: { type: 'object', properties: { decision: { type: 'string' }, blockers: { type: 'array', items: { type: 'string' } }, checklist: { type: 'array', items: { type: 'string' } } }, required: ['decision'] } })
