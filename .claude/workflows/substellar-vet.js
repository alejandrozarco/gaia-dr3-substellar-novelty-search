...
phase('Verdict')
const verdict = await agent(`Substellar verdict for Gaia DR3 ${SID}. Mass: ${JSON.stringify(mass)}. Photometry: ${JSON.stringify(phot)}. Novelty: ${JSON.stringify(novelty)}. Classify: super-Jupiter / brown-dwarf / stellar-at-low-inclination / ambiguous. Default conservative — do NOT claim a planet if a low-inclination stellar companion is allowed and the photometry can't rule it out. Output verdict, M2 range (face-on minimum → inclination-marginalized), P(substellar), is_novel(bool), and the best follow-up.`, { label: 'verdict', phase: 'Verdict', schema: { type: 'object', properties: { verdict: { type: 'string' }, M2: { type: 'string' }, P_substellar: { type: 'string' }, is_novel: { type: 'boolean' }, followup: { type: 'string' } }, required: ['verdict'] } })
