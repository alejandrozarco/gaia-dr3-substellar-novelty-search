...
| Gaia-XP WD spectral class (García-Zamora+ 2023) | VizieR `J/A+A/679/A127` → known-object store | Gaia DR3 XP white-dwarf spectral classification (100-pc table: 12,351 WD incl. 257 DZ). Ingested via `scripts/known_objects/ingest_xp_catalogs.py` (front-filter novelty gate for XP-at-scale). The paper's ~79k full candidate pool is not VizieR-tabulated. |
| Gaia-XP carbon stars (Ye+ 2025) | VizieR `J/A+A/697/A107` → known-object store | 451 high-confidence XP C-star candidates; same ingest script. The ~43k Roulston+ 2025 catalogue (`J/ApJ/982/184`) is NOT in VizieR as of 2026-06-04 — the script has a `ROULSTON_VIZIER` hook to add it when CDS publishes. |
