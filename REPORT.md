...
9. **The planet-regime (M_2 < 13 M_J) tail of the Gaia DR3 NSS Orbital pool is observationally tractable for first-pass detection but requires new RV observations outside the multi-pipeline cross-match framework used for the substellar candidates.** Applying the Stage-1 face-on mass cut narrowly (M_2 face-on < 13 M_J) to the NSS Orbital pool yields 39 candidates, of which 10 are already-published Gaia-validated systems (Gaia-4, Gaia-5, HD 81040 b, HD 40503 b, HD 132406 b, etc.) and 29 are not in NASA Exoplanet Archive PS. After applying marginalized-mass confidence cuts (M_2 median < 13 M_J AND 1σ upper bound < 30 M_J), 19 novel candidates remain. After SIMBAD stellar-multiple flags (WDS, Tokovinin MSC, SB\*, EB\*) — 0 caught, leaving 19. Cross-match to exoplanet.eu within 30 arcsec — 0 hits. Cross-match to HARPS RVBank within 10 arcsec — 0 hits. Of these 19, 8 pass the tier-A astrometric quality cut (RUWE < 2, ipd_frac ≤ 1, NSS significance > 10). The brightest tier-A candidate is \* 54 Cas (HIP 10031, Gaia DR3 522135261462534528): F8V dwarf at 27 pc, G = 6.45, RUWE = 0.88, ipd_frac = 0, NSS Orbital P = 401 d, e = 0.22, marginalized M_2 = 7.3 M_J, predicted K_RV = 183 m/s — observable from any northern spectrograph (HARPS-N, SOPHIE, NEID, FIES, HIRES) at single-night precision. **These planet-regime candidates are single-pipeline detections (Gaia DR3 NSS Orbital only) and lack the multi-archive astrometric corroboration (HGCA, Kervella PMa, Tycho-Gaia ΔPM) used for the substellar tentative list in §4.2.** They are documented separately as the result of the planet-regime probe; confirmation requires dedicated RV programs at northern (\* 54 Cas) or southern (UCAC4 280-221518, UCAC4 240-004042) spectrographs, which are outside the scope of this archival-only pipeline. The empirical implication is that planet-regime astrometric candidates in Gaia DR3 NSS Orbital are within reach with **available archival data** (a single Gaia detection per candidate) but they cannot be elevated to multi-pipeline tentative status until new observations are added.
