...
| object (Gaia DR3 source_id) | estimate | how found | confirm / refute |
|---|---|---|---|
| **WDJ020915** · 332248057157474176 | DA white dwarf (M₁≈0.72 M⊙) + **dark companion M₂≈1.32 [1.27–1.38] M⊙**, P=275 d, M_tot≈2.04 M⊙ | same method as WDJ060042 | provisional — the astrometric orbit is a **weak fit** (F2=+8.39, RUWE=8.79), so the mass is uncertain; Gaia DR4 is decisive. **2026-06-11: first-ever light curve (ATLAS, 10.4 yr) is QUIET** — no outbursts, nothing at P_orb or P/2 → the weak fit is *not* variability-induced (orbit-reality modestly strengthened) **2026-09-22: no radio (NVSS, VLASS, LoTSS-DR2, TGSS), pulsar (ATNF), gamma-ray (4FGL-DR4) or X-ray (2RXS) counterpart, coverage proved; at 84 pc a pulsar companion beamed toward us is excluded.** |
| **1eRASS J152614.8−111331** · 6315134987927550592 | white dwarf + M dwarf binary candidate (possibly a low-accretion-rate polar); P = 2.2511 h (2.4848 h alias not excluded); d = 121 pc; M_G = 12.12 at BP−RP = 2.30, 2.5 mag below the main sequence; blue g−r (SkyMapper 0.25, PS1 0.46) with red r−i (1.3–1.5); eRASS:3 L_X ≈ 1×10²⁹ erg/s | eROSITA eRASS1 × Gaia × PS1 screen (2026-07), re-examined 2026-09-22: the July 'active M dwarf, no period' verdict phased an alias | an optical spectrum (Balmer emission, cyclotron humps, white-dwarf features). The same screen recovered the catalogued polar candidate BLVS J061325.64−030239.2 (VSX AM:) at its known period. |
...
|---|---|---|---|
| 1593152388271709824 | G-subgiant + **dark M₂≈1.27 M⊙** (white-dwarf / neutron-star boundary); NSS significance 104.5 | NSS Orbital + a weak (~3σ) archival-RV corroboration | Tier-2; needs a clean RV epoch. **2026-09-22: TESS eclipsing signal at P = 0.6777712 d centred on the star (VSX submission 2026-09-22, pending); listed in Green+2023 table 2 and Kostov+2025 table 2.** **2026-09-22 (later): TRIPLE-FAVOURED — the 0.678-d eclipsing pair (two K dwarfs: R₁+R₂ ≥ 1.3 R⊙, ~2–3% of the light each) accounts for the dark companion's mass and light budget; the eclipse timing is consistent with this within spot-driven errors. No compact companion is required.** |
...
| **Tier-1 NS pool** (after M_1 correction) | **160** (was 291 at fixed M_1=1.5; 131 demoted to WD/low-mass at real M_1). **F#33 (2026-05-31) down-tiers a further 79 AstroSpectroSB1 + 1 BH** whose Gaia NSS period is flagged non-significant (flags bit 13) → "Tier-2 (period non-significant)"; raw-v2 Tier-1 NS parquet count drops 277→198. **2026-06-10: F#33 propagated into the corrected tiers** (`tier_v2_corrected_f33`, `scripts/apply_f33_to_corrected_tiers_2026_06_10.py`): corrected Tier-1 NS 148 → **107** (main) and 13 → **9** (relaxed) — policy-consistent triaged Tier-1 NS roster = **116**. The DR4 rehunt diff must baseline on `tier_v2_corrected_f33`. **2026-09-22: archival-RV triage re-run reproduces 117/40/2/2 and recovers the 8 lost verdicts (all NO_ARCHIVAL_RV; `docs/reports/ns_pool_triage_rerun_2026_09_22/`). 9 of the pool are in Shahaf+2024 (arXiv:2309.15143) table I; 8 have a red-colour-excess probability ≥ 70% (triple-favoured): 319869338637148928, 348572609373952768, 1841935590054051328, 1903279027392243968, 5136025521527939072, 5446310318525312768, 5815822813333805056, 6545745216706898560; 3494029910469026432 at 64%.** |
| Tier-1 BH pool | **0 STRONG** — 5406907 downgraded to weak/suspect (orbit DPAC-rejected + Gaia RV non-detection); HD 157033 BH-claim now ambiguous (0.4–6 M⊙) **2026-09-22: 6281177228434199296 has a 99.99% red-colour-excess probability in Shahaf+2024 table I (triple-favoured).** |
| Tier-2 (RV-inconclusive) | 426 (after M_1 correction; was 958) |
| v3 Acceleration P-degenerate compact-object | 10,818 at selection; **7,062 surviving Tier-2** after the 2026-05-31 RV-joint demotion of 3,761 (2026-06-10 data audit). Still **not M_1-corrected** — all 16,949 rows at default M_1=1.5; ~24% of surviving Tier-2 would drop below the 1.2 M⊙ NS floor at M_1≈1.0 (queued debt, see RESEARCH_LOG 2026-06-10) |
...
**Real (survives):** a dwarf nova with 6 ZTF + 1 CRTS outbursts and a genuine 2023 superoutburst (7-day, g = 14.41) → SU UMa subtype. **No orbital period, eclipse, or inclination.** CV identity is prior (Drake+ 2014, Coppejans+ 2016, and **Gaia DR3 variability classifier best_class = CV, 0.911** — previously only the DSC QSO false-positive was cited). VSX entry not found in the Vizier `B/vsx` mirror (reconcile vs live AAVSO VSX). Remaining result: the outburst/superoutburst history; the eclipsing period originally claimed is withdrawn. Dossier: `docs/dossiers/CRTS_J051419+0111_DOSSIER_2026_05_28.md` (body §3/§4/§6 numbers superseded by the retraction banner at its head).
...
> **Pipeline note — no global RUWE gate (cascade-regression, 2026-05-30).** The bulk consumer (`consumer_v2.derive_row_v2`) applies no astrometric-quality (RUWE) cut, so two RUWE > 6 sources reach Tier-1: GALEX J145250 (RUWE = 6.46 — the catalog's only Tier-1 BH) and GALEX J033455 (RUWE = 9.35, since published as a WD). Both are individually flagged/demoted in the table above, and the corrected headline makes **no** astrometry-only BH/Tier-1 claim, so this affects no current result. But any *future* Tier-1/BH headline must re-vet on RUWE first; adding a global RUWE flag to the tiering is a tracked follow-up. **DONE 2026-06-10 — as F#34** (`consumer_v2.filter34_astromqual`; a caution *flag*, never a cut): empirically, the whole corrected Tier-1 pool has RUWE 2.6–28.8 (median 6.1 — elevated single-star RUWE is the *normal* photocentric-binary signature, so an absolute RUWE cut discriminates nothing; the two motivating offenders sit inside the bulk). F#34 instead fires on the NSS orbit's own goodness-of-fit **F2 > +5** (the frozen prereg "unreliable" line), `ipd_frac_multi_peak ≥ 4` (resolved-double contamination), or RUWE ≥ 12.5 (Tier-1 P95). Applied to all three `_M1corrected` parquets (`filter34_v2`/`filter34_reason_v2` + new `nss_gof_f2`/`ipd_frac_multi_peak` columns; fetch archived at `docs/reports/f2_ipd_fetch_2026_06_10.csv`): **51/108 main + 3/9 relaxed Tier-1 NS carry the caution — including GALEX J145250 (F2=+8.0), the case that motivated this note.** Cascade-regression gate after the change: PASS (no behavioral regression; 63/63 tests; 6/6 negative controls).
...
Cascade validation: 93% class-level recall on 27 confirmed compact + sub-stellar systems, 14% adversarial-FP rate, 4.5% median |ΔM_2|/M_2 on the Shahaf+ 2024 NS sample. **⚠ 2026-06-10: treat the recall figure as historical/unverified** — the 27-system validation set was never committed to the repo (and CITATION.cff says "70 systems"), so the 2026-06-10 cascade-regression gate could not re-score it. Either re-commit the frozen truth set or soften this claim before any release. (The gate's reproducible checks all pass: frozen production tiers exact, 6/6 negative-control specificity, BH2 fixture Tier-1.)
