...
def analyse(t):
    ...
    if m1.sum() > 60 and (~m1).sum() > 60:
        ...
        j = int(np.argmax(r1.power)); P1 = float(r1.period[j]); t01 = float(r1.transit_time[j])
        ph2 = ((mjd[~m1] - t01)/P1) % 1.0; ph2[ph2 > 0.5] -= 1.0
        ine2 = np.abs(ph2) < (dur/P)/2*1.2
        d2 = float(np.median(mag[~m1][ine2])) - base if ine2.sum() >= 3 else 0.0
        rec["holdout_P"] = round(P1,6); rec["holdout_n_inecl"] = int(ine2.sum())
        rec["holdout_depth"] = round(d2,4)
        rec["holdout_pass"] = bool(ine2.sum() >= 3 and d2 > 0.5*depth
                                   and abs(P1-P)/P < 0.02)
    else:
