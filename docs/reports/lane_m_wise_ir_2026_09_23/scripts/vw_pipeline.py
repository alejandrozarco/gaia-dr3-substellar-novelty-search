# Automated IR-vs-optical amplitude screen for high-latitude VarWISE variables below the main sequence.
# Output: vw_pipeline.jsonl (one line per target; status OK / HOLE_* recorded, never silently dropped)
...
try:
    for l in open("vw_pipeline.jsonl"): done.add(json.loads(l)["designation"])
...
def work(r):
    ...
    if de0 > -29:
        ...
        if not q.startswith("oid"): out["ztf"] = "HOLE"
            ...
            Z = [z for z in csv.DictReader(io.StringIO(q)) if z["catflags"] == "0" and abs(float(z["sharp"])) < 0.5 and float(z["mag"]) < float(z["limitmag"]) - 0.2]
...
with ThreadPoolExecutor(max_workers=3) as ex:
    for o in ex.map(work, todo):
        with lock:
            open("vw_pipeline.jsonl", "a").write(json.dumps(o) + "\n")
            flag = ""
