# Generic WISE W1 bright-minus-faint localisation (generalised from j0753_localise.py, 2026-09-23).
# Usage: python wise_localise.py <gaia_dr3_source_id> <period_d>. NEOWISE-R W1 detections (IRSA TAP, lane M clean cuts) define the
# 4-harmonic phase template; NEOWISE-R W1 L1b cutouts (IRSA IBE) are stacked at plateau (level > 0.8) and dip (level < 0.25) phases.
import sys, os, io, csv, json, subprocess, time, warnings, urllib.parse, numpy as np
warnings.filterwarnings("ignore")
SID = sys.argv[1]; P = float(sys.argv[2]); TAG = f"s{SID}"
def gaia(q):
    u = "https://gea.esac.esa.int/tap-server/tap/sync?REQUEST=doQuery&LANG=ADQL&FORMAT=csv&QUERY=" + urllib.parse.quote(q)
    return list(csv.DictReader(io.StringIO(subprocess.run(["curl", "-sL", "--max-time", "180", u], capture_output=True, text=True).stdout)))
g = gaia(f"SELECT ra, dec, pmra, pmdec FROM gaiadr3.gaia_source WHERE source_id={SID}")[0]
RA16, DE16, PMRA, PMDE = float(g["ra"]), float(g["dec"]), float(g["pmra"] or 0), float(g["pmdec"] or 0)
def irsa(q):
    for k in range(6):
...
det = []
for x in irsa(f"SELECT mjd, ra, dec, w1mpro, w1sigmpro, qual_frame, qi_fact, saa_sep, moon_masked, cc_flags, nb, na FROM neowiser_p1bs_psd "
              f"WHERE CONTAINS(POINT('ICRS',ra,dec),CIRCLE('ICRS',{RA16},{DE16},{5/3600}))=1"):
    try:
        yr = (float(x["mjd"]) - 57388.0) / 365.25; rap = RA16 + PMRA * yr / 3.6e6 / np.cos(np.radians(DE16)); dep = DE16 + PMDE * yr / 3.6e6
        if np.hypot((float(x["ra"]) - rap) * np.cos(np.radians(DE16)), float(x["dec"]) - dep) * 3600 > 3: continue
        ...
        det.append((float(x["mjd"]), float(x["w1mpro"]), float(x["w1sigmpro"])))
    except ValueError: pass
det = np.array(det)
os.makedirs(TAG, exist_ok=True); os.chdir(TAG)
if not os.path.exists("frames_neo.tbl") or os.path.getsize("frames_neo.tbl") < 1000:
    subprocess.run(["curl", "-s", "--max-time", "300", "-o", "frames_neo.tbl", f"https://irsa.ipac.caltech.edu/ibe/search/wise/neowiser/p1bm_frm?POS={RA16},{DE16}&where=band%3D1"])
...
tb = bjd(det[:, 0]); Fd = 10 ** (-0.4 * (det[:, 1] - 15.0)); w = det[:, 2]
ph = (tb / P) % 1
...
res = dict(object=f"Gaia DR3 {SID}", period_d=P, n_detections=int(len(det)), epoch=EP, n_frames=int(len(imgs)), n_bright=int(bright.sum()), n_faint=int(faint.sum()),
...
json.dump(res, open(f"localise_{SID}.json", "w"), indent=1); print(json.dumps({k: v for k, v in res.items() if k != "reg_star_offsets"}, indent=1))
...
fig.suptitle(f"Gaia DR3 {SID} NEOWISE W1 (P = {P} d): red + = Gaia DR3 position at 2019.5; blue x = difference-image centroid", fontsize=10)
plt.tight_layout(); plt.savefig(f"localise_{SID}.png", dpi=110)
# --- scan-direction systematics: target and Gaia field stars (G < 19.5) in the MEAN images per scan direction
np.savez_compressed(f"stack_{SID}.npz", imgs=imgs, mjds=mjds, crotas=crotas, lev=lev)
...
for lab, m in (("cos>0", asc), ("cos<0", ~asc)):
    ...
    t = cen_at(Am, 0, 0)
    rows.append((lab, "target (mean image)", 0.0, 0.0, None if t is None else (round(float(t[1]), 2), round(float(t[2]), 2))))
