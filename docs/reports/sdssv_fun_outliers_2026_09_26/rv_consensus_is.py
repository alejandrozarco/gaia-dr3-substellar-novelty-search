...
d = pd.read_csv("rv_multi_all.csv", dtype={"sdss_id": str}); d = d[(d.rv.abs() < 690) & (d.in_stack == True)]
rows = []
...
O = pd.DataFrame(out); O.to_csv("rv_consensus_instack.csv", index=False); print(len(O))
