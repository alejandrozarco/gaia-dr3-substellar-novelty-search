...
for i in range(len(ids)):
    ...
    for j in NB[i, 1:11]:
        ...
        for a, b in zip(edges[:-1], edges[1:]):
            ...
            s = np.sum(xa * ya / va) / np.sum(ya * ya / va); chi_parts.append((float(np.sum((xa - s * ya) ** 2 / va)), int(mm.sum())))
...
out.sort_values("uniqueness", ascending=False).to_csv("weird_mf.csv", index=False); print("DONE", flush=True)
