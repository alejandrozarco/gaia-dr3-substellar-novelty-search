# Lane 1b (earlier sectors < 70, Cycles 1-5): SDSS-V DR20 white dwarfs in the ZZ Ceti strip (SnowWhite DA, Teff 10-13.5 kK, log g 7.3-9.3) and the DBV strip
...
# Output lane1b_lc_results.jsonl (one row per light curve). Holes (failed downloads/reads) are recorded, never dropped.
...
if os.path.exists("lane1b_lc_results.jsonl"):
    done = {json.loads(l)["file"] for l in open("lane1b_lc_results.jsonl")}
todo = [x for x in todo if x[2] not in done]
with ThreadPoolExecutor(max_workers=4) as ex, open("lane1b_lc_results.jsonl", "a") as out:
    for i, (item, path) in enumerate(zip(todo, ex.map(fetch, todo))):
        row = analyse(item, path); out.write(json.dumps(row) + "\n"); out.flush()
        if path and os.path.exists(path) and not (row.get("status") == "OK" and row["peaks"][0][2] >= 4.5): os.remove(path)   # disk is nearly full: keep only files with a >= 4.5 S/N peak
        if i % 50 == 0: print(f"{time.strftime('%H:%M:%S')} {i}/{len(todo)} {row['status']}", flush=True)
print("LANE1B_DONE", flush=True)
