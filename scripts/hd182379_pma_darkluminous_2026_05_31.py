...
Gaia DR3 2046110638731786240 = HD 182379 (2026-05-31).

Caller context: HGCA chi2=176.1, Kervella snrPMa=15.79, M2_median (Kervella
M2_5AU MC) = 2.637 Msun, SIMBAD HD 182379, known_multiple=true. This is a PMa /
HGCA *acceleration* source -- NOT a Thiele-Innes orbit and (per METHODOLOGY.md)
the Acceleration-NSS extension is the deferred channel: M2 comes from the
proper-motion anomaly + assumed period, not an algebraic mass-function inversion.
...
*conspicuous* unless the primary itself is far more luminous. So:
  ...
  (4) Is Kervella M2_5AU=2.637 + HGCA chi2=176.1 consistent with a *luminous*
      stellar companion of that mass, or does a DARK companion remain required?
      Decisive cross-check: the M2~2.6 Msun A-star's delta-G vs the primary and
...
Outputs: /tmp/hd182379_pma.json , /tmp/hd182379_pma_report.md
...
def run():
    print('=' * 78)
    print(f'GAIA DR3 {SID} = HD 182379 -- PMa dark-vs-luminous companion vetting')
    print('=' * 78)
    ...
    sim = simbad_otype_bibs(f'Gaia DR3 {SID}', hd_name='HD 182379')
    hd_name = 'HD 182379'
    ...
    # M2_median (caller) = 2.637 -> approx A1-A2V (Teff~8800-9300, R~2.05-2.14).
    ...
    result = dict(
        ...
        caller_inputs=dict(HGCA_chi2=176.1, snrPMa=15.79, M2_median=2.637,
                           known_multiple=True),
    ...
    with open('/tmp/hd182379_pma.json', 'w') as f:
    ...
    print("\n================ KERVELLA M2_median COMPANION (2.637 Msun A-star) ================")
    em = e_med
    print(f"  Assumed luminous companion at M2=2.637 Msun: SpT~A1-A2V, T2={em['T2_assumed']:.0f} K, "
