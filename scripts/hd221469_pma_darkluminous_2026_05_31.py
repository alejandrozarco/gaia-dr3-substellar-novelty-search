...
Gaia DR3 2865472476175772672 = HD 221469 (2026-05-31).

Caller context: HGCA chi2=2614, Kervella snrPMa=35.49, M2_median (Kervella
M2_5AU MC) = 2.358 Msun, SIMBAD HD 221469, known_multiple=true. This is a PMa /
...
*conspicuous* unless the primary itself is far more luminous. So:
  ...
  (4) Is Kervella M2_5AU=2.358 + HGCA chi2=2614 consistent with a *luminous*
      stellar companion of that mass, or does a DARK companion remain required?
      Decisive cross-check: the M2~2.36 Msun A-star's delta-G vs the primary and
...
Outputs: /tmp/hd221469_pma.json , /tmp/hd221469_pma_report.md
...
hd182379_pma_darkluminous (the direct template).
"""
...
SID = 2865472476175772672
HD_NAME = 'HD 221469'
CALLER = dict(HGCA_chi2=2614, snrPMa=35.49, M2_median=2.358, known_multiple=True)
...
def gaia_nss_accel(sid):
    """nss_acceleration_astro: the Acceleration7/Acceleration9 channel (3<P<~1000d,
    the deferred-extension home of pure PMa sources). If a row exists it constrains
    the acceleration directly (and tells us this is NOT a too-long-period orbit)."""
    def go():
        return Gaia.launch_job(
            f'SELECT * FROM gaiadr3.nss_acceleration_astro WHERE source_id={sid}').get_results()
    t = _with_timeout(go, 60, None)
    return _tab_to_dict(t) if (t is not None and len(t)) else None
...
def run():
    ...
    # M2_median (caller) = 2.358 -> approx A2-A5V (Teff~8000-8800, R~1.8-2.05).
    ...
    result = dict(
        ...
        gaia_source=gs, gaia_ap=ap, gaia_ap_supp=aps, nss=nss, nss_accel=nss_acc,
    ...
    with open('/tmp/hd221469_pma.json', 'w') as f:
    ...
    print(f"  NSS orbit row        = {nss is not None} ({nss.get('nss_solution_type') if nss else '-'})")
    if nss:
        print(f"      NSS P={nss.get('period')} d  e={nss.get('eccentricity')}  "
              f"plx_nss={nss.get('parallax')}  a_TI(A,B,F,G)="
              f"{nss.get('a_thiele_innes')},{nss.get('b_thiele_innes')},"
              f"{nss.get('f_thiele_innes')},{nss.get('g_thiele_innes')}")
    print(f"  NSS accel row        = {nss_acc is not None} ({nss_acc.get('nss_solution_type') if nss_acc else '-'})")
    if nss_acc:
        for k in ('accel_ra','accel_dec','deriv_accel_ra','deriv_accel_dec',
                  'significance','period'):
            if k in nss_acc:
                print(f"      accel {k:18s} = {nss_acc.get(k)}")
    ...
    em = e_med
    print(f"  Assumed luminous companion at M2={M2_med} Msun: SpT~A2-A5V, T2={em['T2_assumed']:.0f} K, "
    ...
    if kerv.get('match'):
        # NB: Kervella J/A+A/657/A7 secondary-mass columns (M25au etc.) are in JUPITER
        # masses; divide by 1047.57 for Msun. Primary M1 is in Msun.
        for k in ('sep_arcsec', 'HD', 'HIP', 'GaiaEDR3', 'M1', 'e_M1',
                  'M25au', 'e_M25au', 'E_M25au', 'M23au', 'M210au', 'M230au',
                  'dVt', 'e_dVt', 'snrPMaH2G2', 'snrPMaHG1', 'snrPMaH2EG3b', 'Plx'):
            if k in kerv:
                print(f"    Kervella {k:14s} = {kerv.get(k)}")
        for col, label in (('M25au','M2_5AU'),('M23au','M2_3AU'),('M210au','M2_10AU')):
            v = kerv.get(col)
            if isinstance(v, (int, float)):
                print(f"    -> Kervella {label} = {v:.1f} M_Jup = {v/1047.57:.3f} Msun")
    else:
