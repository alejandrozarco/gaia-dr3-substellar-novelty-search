"""Wider hunt — RA-chunked. G<14, plx>=0.5, sig>=30."""
...
def fetch_ra_chunk(ra_min, ra_max):
    ...
    SELECT nss.source_id, nss.nss_solution_type, nss.period, nss.eccentricity,
           ...
           g.parallax, g.phot_g_mean_mag, g.bp_rp, g.ruwe,
    ...
    for attempt in range(3):
        try:
            ...
            print(f'  attempt {attempt+1}: ERR {type(e).__name__}: {str(e)[:80]}')
            time.sleep(10)
    return None
...
def main():
    ...
    for ra_min in range(0, 360, 30):
        ra_max = ra_min + 30
        t0 = time.time()
        print(f'Fetching RA {ra_min}-{ra_max} ...', flush=True)
    ...
    src_ids = pool['source_id'].to_list()
    sb2_ids = set()
    for i in range(0, len(src_ids), 3000):
        sub = src_ids[i:i+3000]
    ...
    print('Pulling FLAME + Teff + log g ...')
    chunks_f = []
    for i in range(0, len(src_ids), 3000):
        sub = src_ids[i:i+3000]
        ids = ','.join(str(int(x)) for x in sub)
