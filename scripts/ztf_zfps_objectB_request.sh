...
wget --http-user=ztffps --http-passwd=dontgocrazy \
  -O /tmp/zfps_objectB_submit_response.html \
  "https://ztfweb.ipac.caltech.edu/cgi-bin/requestForcedPhotometry.cgi?ra=${RA}&dec=${DEC}&jdstart=${JDSTART}&jdend=${JDEND}&email=${EMAIL}&userpass=${ZFPS_PASS}"

echo "--- response ---"
grep -oiE "(success|error|exceed|invalid)[^<]*" /tmp/zfps_objectB_submit_response.html || cat /tmp/zfps_objectB_submit_response.html
echo
echo "Submitted: Object B (${RA} ${DEC}), full survey window. Results arrive by email (can take 7+ days)."
